...
-- This allows us to process data for any date range
  ...
  WHILE current_hour <= end_date LOOP
    ...
    LOOP
      ...
      SELECT
        ...
        COUNT(DISTINCT p.id) as total_entries,
        COUNT(DISTINCT p.id) as total_exits,
        SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'in' THEN p.count ELSE 0 END) as store_entries,
        SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'out' THEN p.count ELSE 0 END) as store_exits,
        SUM(CASE WHEN p.line_id = 4 THEN p.count ELSE 0 END) as passerby_count,
        SUM(CASE WHEN p.line_id = 4 AND p.direction = 'in' THEN p.count ELSE 0 END) as passerby_in,
        SUM(CASE WHEN p.line_id = 4 AND p.direction = 'out' THEN p.count ELSE 0 END) as passerby_out,
        -- Capture rate
        CASE 
          WHEN SUM(CASE WHEN p.line_id = 4 THEN p.count ELSE 0 END) > 0 THEN
            (SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'in' THEN p.count ELSE 0 END)::DECIMAL / 
             SUM(CASE WHEN p.line_id = 4 THEN p.count ELSE 0 END)::DECIMAL * 100)
          ELSE 0
        END as capture_rate,
        -- Entry distribution percentages
        CASE 
          WHEN SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'in' THEN p.count ELSE 0 END) > 0 THEN
            ROUND(SUM(CASE WHEN p.line_id = 1 AND p.direction = 'in' THEN p.count ELSE 0 END)::DECIMAL / 
                  SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'in' THEN p.count ELSE 0 END)::DECIMAL * 100, 2)
          ELSE 0
        END as entry_line1_pct,
        CASE 
          WHEN SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'in' THEN p.count ELSE 0 END) > 0 THEN
            ROUND(SUM(CASE WHEN p.line_id = 2 AND p.direction = 'in' THEN p.count ELSE 0 END)::DECIMAL / 
                  SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'in' THEN p.count ELSE 0 END)::DECIMAL * 100, 2)
          ELSE 0
        END as entry_line2_pct,
        CASE 
          WHEN SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'in' THEN p.count ELSE 0 END) > 0 THEN
            ROUND(SUM(CASE WHEN p.line_id = 3 AND p.direction = 'in' THEN p.count ELSE 0 END)::DECIMAL / 
                  SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'in' THEN p.count ELSE 0 END)::DECIMAL * 100, 2)
          ELSE 0
        END as entry_line3_pct,
        -- Exit distribution percentages
        CASE 
          WHEN SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'out' THEN p.count ELSE 0 END) > 0 THEN
            ROUND(SUM(CASE WHEN p.line_id = 1 AND p.direction = 'out' THEN p.count ELSE 0 END)::DECIMAL / 
                  SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'out' THEN p.count ELSE 0 END)::DECIMAL * 100, 2)
          ELSE 0
        END as exit_line1_pct,
        CASE 
          WHEN SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'out' THEN p.count ELSE 0 END) > 0 THEN
            ROUND(SUM(CASE WHEN p.line_id = 2 AND p.direction = 'out' THEN p.count ELSE 0 END)::DECIMAL / 
                  SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'out' THEN p.count ELSE 0 END)::DECIMAL * 100, 2)
          ELSE 0
        END as exit_line2_pct,
        CASE 
          WHEN SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'out' THEN p.count ELSE 0 END) > 0 THEN
            ROUND(SUM(CASE WHEN p.line_id = 3 AND p.direction = 'out' THEN p.count ELSE 0 END)::DECIMAL / 
                  SUM(CASE WHEN p.line_id IN (1,2,3) AND p.direction = 'out' THEN p.count ELSE 0 END)::DECIMAL * 100, 2)
          ELSE 0
        END as exit_line3_pct,
        -- Line details
        SUM(CASE WHEN p.line_id = 1 AND p.direction = 'in' THEN p.count ELSE 0 END) as line1_in,
        SUM(CASE WHEN p.line_id = 1 AND p.direction = 'out' THEN p.count ELSE 0 END) as line1_out,
        SUM(CASE WHEN p.line_id = 2 AND p.direction = 'in' THEN p.count ELSE 0 END) as line2_in,
        SUM(CASE WHEN p.line_id = 2 AND p.direction = 'out' THEN p.count ELSE 0 END) as line2_out,
        SUM(CASE WHEN p.line_id = 3 AND p.direction = 'in' THEN p.count ELSE 0 END) as line3_in,
        SUM(CASE WHEN p.line_id = 3 AND p.direction = 'out' THEN p.count ELSE 0 END) as line3_out,
        SUM(CASE WHEN p.line_id = 4 AND p.direction = 'in' THEN p.count ELSE 0 END) as line4_in,
        SUM(CASE WHEN p.line_id = 4 AND p.direction = 'out' THEN p.count ELSE 0 END) as line4_out,
