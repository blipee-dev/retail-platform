...
-- Create helper function for organization-based access
-- This function will be used by our server-side API with service role
...
CREATE POLICY "sensor_data_select_org" ON sensor_data
    FOR SELECT USING (
        sensor_id IN (
            SELECT id FROM sensors WHERE organization_id = auth.get_user_organization_id()
        )
    );
