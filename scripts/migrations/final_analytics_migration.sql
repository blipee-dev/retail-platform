...
-- This version adds all missing columns to hourly_analytics
...
-- =====================================================
-- 2. ALTER HOURLY ANALYTICS TABLE - ADD ALL MISSING COLUMNS
-- =====================================================

-- Add all missing columns to hourly_analytics
...
-- Update date and hour from hour_start if they're null
UPDATE hourly_analytics 
SET date = DATE(hour_start),
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'unique_hourly_analytics'
    ) THEN
        ALTER TABLE hourly_analytics 
        ADD CONSTRAINT unique_hourly_analytics UNIQUE (sensor_id, date, hour);
    END IF;
END $$;

-- =====================================================
-- 3. CREATE SIMPLIFIED HOURLY AGGREGATION FUNCTION
-- =====================================================
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO hourly_analytics (
            ...
            v_record.store_id,
            v_record.sensor_id,
            v_record.hour_timestamp,
            v_date,
            v_hour,
            v_record.total_in,
            v_record.total_out,
            v_record.total_in,  -- total_entries
            v_record.total_out, -- total_exits
            v_record.total_in - v_record.total_out, -- net_flow
            v_record.line1_in,
            v_record.line1_out,
            v_record.line2_in,
            v_record.line2_out,
...
-- =====================================================
-- 4. CREATE DAILY AGGREGATION FUNCTION
-- =====================================================
...
BEGIN
    ...
    FOR v_record IN 
        SELECT 
            ...
            sensor_id,
            date,
            SUM(COALESCE(total_in, 0)) as total_in,
            SUM(COALESCE(total_out, 0)) as total_out,
            AVG(COALESCE(total_in, 0)) as avg_hourly_in,
            AVG(COALESCE(total_out, 0)) as avg_hourly_out,
            -- Aggregate business hours (9 AM - 9 PM)
            SUM(CASE WHEN hour >= 9 AND hour < 21 THEN COALESCE(total_in, 0) ELSE 0 END) as business_hours_in,
            SUM(CASE WHEN hour >= 9 AND hour < 21 THEN COALESCE(total_out, 0) ELSE 0 END) as business_hours_out
        FROM hourly_analytics
    ...
    LOOP
        ...
        -- Find peak hour
        SELECT hour, COALESCE(total_in, 0) + COALESCE(total_out, 0)
        INTO v_peak_hour, v_peak_traffic
        FROM hourly_analytics
        WHERE sensor_id = v_record.sensor_id AND date = v_record.date
        ORDER BY (COALESCE(total_in, 0) + COALESCE(total_out, 0)) DESC
        LIMIT 1;
        
        -- Handle case where no peak hour is found
...
CREATE INDEX IF NOT EXISTS idx_hourly_analytics_date ON hourly_analytics(date, hour);
